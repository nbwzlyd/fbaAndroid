...
public class CustomDrawable {
	...
	public CustomDrawable(Context ctx, int resid, int button_id) {
		...
		drawable.setBounds(0, 0, drawable.getBitmap().getWidth(), drawable
				.getBitmap().getHeight());
	...
	public void setCenter(int x, int y) {
		bounds.set(x - bounds.width() / 2, y - bounds.height() / 2,
	...
	public void setScale(float _scale) {
		...
		// Utility.log(
		// "setScaleOLD: "+getWidth()+"x"+getHeight()+" (scale="+scale+")" );
		...
		bounds.set(
				...
				bounds.top,
				bounds.left
						+ (int) ((float) drawable.getBitmap().getWidth() * scale),
				bounds.top
						+ (int) ((float) drawable.getBitmap().getHeight() * scale));

		// Utility.log(
		// "setScaleNEW: "+getWidth()+"x"+getHeight()+" (scale="+scale+")" );
	...
	public boolean load(int button_count, int orientation) {
		...
		data = (ButtonData) Utility.unserialize(prefs.getString(
				createID(orientation), null));
		if (data != null) {
			Utility.log("custom configuration found for: " + SDLActivity.rom
		...
		data = (ButtonData) Utility.unserialize(prefs.getString(
				createIDDefault(button_count, orientation), null));
		if (data != null) {
			Utility.log("default configuration found for buttons: "
					+ button_count + ", orientation: " + orientation);
			setCenter(data.x, data.y);
			setScale(data.s);
			setAlpha(data.a);
			return true;
	...
	public void save(int button_count, int orientation) {
		...
		{
			Utility.log(createID(orientation) + " -> x=" + getCenter().x
					+ ":y=" + getCenter().y + ":s=" + scale + ":a=" + alpha);

			if (button_count > -1) {
				if (!prefs
						.edit()
						.putString(createIDDefault(button_count, orientation),
								getData()).commit())
					Utility.loge("save input configuration failed (button_count > -1)");
			} else {
				if (!prefs.edit().putString(createID(orientation), getData())
						.commit())
	...
	private String createID(int orientation) {
		return SDLActivity.rom + String.valueOf(id) + String.valueOf(orientation);
	}
	...
	private String getData() {
		...
		data.x = getCenter().x;
		data.y = getCenter().y;// data.y == getCenter().y ? getCenter().y :
								// getCenter().y + Main.actionBarHeight;
		data.s = scale;
		data.a = alpha;
