package com.codegg.fba.emulator.sdl;
...
public class SDLSurface extends SurfaceView implements SurfaceHolder.Callback,
    ...
    public SDLSurface(Context context, AttributeSet attrs, int defStyleAttr) {
	...
	class SDLGenericMotionListener_API12 implements
	...
	public void surfaceChanged(SurfaceHolder holder, int format, int width,
		...
		boolean skip = false;
		int requestedOrientation = SDLActivity.activity.getRequestedOrientation();
	...
	class SDLMain implements Runnable {
		...
		public void run() {
			// Runs SDL_main()
			SDLActivity.nativeInitWithArgs(SDLActivity.args);
	...
	public boolean onKey(View v, int keyCode, KeyEvent event) {
		...
		// Some SOURCE_JOYSTICK, SOURCE_DPAD or SOURCE_GAMEPAD are also
		// SOURCE_KEYBOARD
		// So, we try to process them as JOYSTICK/DPAD/GAMEPAD events first, if
		// that fails we try them as KEYBOARD
		//
		...
		if ((event.getSource() & InputDevice.SOURCE_MOUSE) != 0) {
			...
			// presses
			// they are ignored here because sending them as mouse input to SDL
			// is messy
			...
			if (Build.VERSION.SDK_INT < 14) {
				...
				try {
					mouseButton = (Integer) event.getClass()
							.getMethod("getButtonState").invoke(event);
				} catch (Exception e) {
					mouseButton = 1; // oh well.
				}
			}
			SDLActivity.onNativeMouse(mouseButton, action, event.getX(0),
					event.getY(0));
		} else {
			switch (action) {
