...
public class romListActivity extends AppCompatActivity implements
        romListFragment.Callbacks {
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        romListFragment = (romListFragment) getSupportFragmentManager()
                .findFragmentById(R.id.rom_list);
    ...
    public void Init(String dataName, ArrayList<RomInfo> romsList) {
        ...
        EmuPreferences.DATA_PATH = Environment.getExternalStorageDirectory()
                .getAbsolutePath() + "/" + dataName;
        EmuPreferences.ROM_PATH = Environment.getExternalStorageDirectory()
                .getAbsolutePath();
        ...
        try {
            new File(EmuPreferences.ICONS_PATH + "/" + ".nomedia")
                    .createNewFile();
            new File(EmuPreferences.PREVIEWS_PATH + "/" + ".nomedia")
                    .createNewFile();
            new File(EmuPreferences.TITLES_PATH + "/" + ".nomedia")
                    .createNewFile();
        } catch (IOException e) {
            e.printStackTrace();
        }
    ...
    public void onBackPressed() {
        ...
        if (!toastShowed) {
            toast = Toast.makeText(this, "Press back twice to exit",
                    Toast.LENGTH_SHORT);
            toast.show();
            toastShowed = true;
        } else if (toast != null) {
    ...
    void fragmentShow(String id) {
        ...
        getSupportFragmentManager().beginTransaction()
                .replace(R.id.rom_detail_container, fragment).commit();
    }
    ...
    private void downloadPreviews() {
        HTTPDownload d = new HTTPDownload(EmuPreferences.DATA_URL,
                EmuPreferences.DATA_PATH + "/data.zip") {
            @Override
            public void onProgress(int current, int total) {
                message.show(
                        "Downloading ... " + Utility.formatFileSize(current)
                                + " / " + Utility.formatFileSize(total) + "\n",
                        current, total);
            }
