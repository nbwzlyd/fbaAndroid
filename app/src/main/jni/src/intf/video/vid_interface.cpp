...
#else
	fbaprintf("VidInit ----Init---nVidSelect  = %d--bDrvOkay = %d",nVidSelect,bDrvOkay);
	if ((nVidSelect < VID_LEN) && bDrvOkay) {
#endif
		nVidActive = nVidSelect;

		fbaprintf("VidInit ----Init----");

		if ((nRet = pVidOut[nVidActive]->Init()) == 0) {
			nBurnBpp = nVidImageBPP;								// Set Burn library Bytes per pixel

			bVidOkay = true;
			printf("VidInit : nBurnBpp= %d",nBurnBpp);
			if (bDrvOkay && (BurnDrvGetFlags() & BDF_16BIT_ONLY) && nVidImageBPP > 2) {
				nBurnBpp = 2;
				printf("VidInit : nVidImageWidth * nVidImageHeight * sizeof(INT16)= %d",nVidImageWidth * nVidImageHeight * sizeof(INT16));
...
#ifdef ANDROID
		UINT8* pDest = (UINT8*)sdlFramebuf->pixels;
#else
...
#ifdef ANDROID
		pBurnDraw = (UINT8*)sdlFramebuf->pixels; 
#else
		pBurnDraw = pVidImage;
#endif
