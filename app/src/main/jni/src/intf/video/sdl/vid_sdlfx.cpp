...
static int BlitFXMakeSurf()
{
	...
	if (nUseSys == 0) {
		sdlsBlitFX[0] = SDL_CreateRGBSurface(0, nGameWidth * nSize, nGameHeight * nSize, sdlsFramebuf->format->BitsPerPixel, sdlsFramebuf->format->Rmask, sdlsFramebuf->format->Gmask, sdlsFramebuf->format->Bmask, sdlsFramebuf->format->Amask);
	...
	if (nDirectAccess == 0) {
		sdlsBlitFX[1] = SDL_CreateRGBSurface(0, nGameWidth * nSize, nGameHeight * nSize, sdlsFramebuf->format->BitsPerPixel, sdlsFramebuf->format->Rmask, sdlsFramebuf->format->Gmask, sdlsFramebuf->format->Bmask, sdlsFramebuf->format->Amask);
...
static int Init()
{
	...
	if (nVidFullscreen) {
		...
		if (	sdlsFramebuf = SDL_CreateRGBSurface(SDL_SWSURFACE, nVidWidth, nVidHeight, nVidDepth,
		...
		if (	sdlsFramebuf = SDL_CreateRGBSurface(SDL_SWSURFACE, nVidWidth*nSize, nVidHeight*nSize, nVidDepth,
#if SDL_BYTEORDER == SDL_LIL_ENDIAN     /* OpenGL RGBA masks */
												 0x000000FF,
												 0x0000FF00, 0x00FF0000, 0xFF000000
#else
